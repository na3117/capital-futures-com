...
# === Tick 接收事件處理 ===
class TickEvent:
    def OnNotifyTicks(self, sMarketNo, sStockidx, nPtr, nTickCount):
        stock_id = sStockidx
        for i in range(nTickCount):
            tick = api.sk_quote.SKQuoteLib_GetStockByIndex(0, stock_id)
            now = datetime.now()
            price = tick.m_nClose / 100.0
...
for stock_id in target_stocks:
    market = api.detect_market(stock_id)
    api.sk_quote.SKQuoteLib_RequestTicks(market, stock_id)
    TICK_HISTORY[stock_id] = []
    print(f"✅ 已訂閱 {stock_id}")

# 註冊 Tick 事件
tick_handler = TickEvent()
comtypes.client.GetEvents(api.sk_quote, tick_handler)
